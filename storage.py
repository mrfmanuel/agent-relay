...
Claim, heartbeat, terminal submission, and recovery share the writer
transaction seam in :mod:`database`. Under PostgreSQL that seam is an
ordinary transaction, so the functions below additionally take explicit
``FOR UPDATE`` / ``FOR UPDATE SKIP LOCKED`` locks on the rows they mutate;
these locking clauses are no-ops under SQLite, which instead gets its
serialization from the whole-database ``BEGIN IMMEDIATE`` reservation.
"""
...
def create_task(sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None) -> dict[str, str]:
    # On SQLite the BEGIN IMMEDIATE writer transaction already serializes this
    # check-then-insert against other writers. PostgreSQL runs it as an
    # ordinary transaction, so two racing requests with the same idempotency
    # key can both pass the pre-check and only one insert wins the unique
    # constraint; the loser looks the row back up instead of failing.
    try:
        with immediate_transaction() as db:
        ...
        if idempotency_key is None:
            raise
        with db_session() as db:
            return _idempotent_task_result(db, sender_id, recipient_id, input_text, idempotency_key)
